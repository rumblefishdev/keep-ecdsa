...
const FullyBackedSortitionPool = contract.fromArtifact('FullyBackedSortitionPool')
const FullyBackedSortitionPoolFactory = contract.fromArtifact('FullyBackedSortitionPoolFactory')
...
describe('FullyBackedECDSAKeepFactory', function () {
  ...
  before(async () => {
    ...
    await FullyBackedSortitionPoolFactory.link('StackLib', (await StackLib.new()).address)
  })
  ...
  describe('registerMemberCandidate', async () => {
    ...
    it('inserts operators to different pools', async () => {
      ...
      const signerPool1Address = await keepFactory.createSortitionPool.call(application1)
      await keepFactory.createSortitionPool(application1)
      const signerPool2Address = await keepFactory.createSortitionPool.call(application2)
      await keepFactory.createSortitionPool(application2)

      await bonding.authorizeSortitionPoolContract(members[0], signerPool1Address, {
        from: authorizers[0],
      })
      await bonding.authorizeSortitionPoolContract(members[1], signerPool2Address, {
        from: authorizers[1],
      })
  ...
  describe('updateOperatorStatus', async () => {
    ...
    it('updates operator if bonding value increased significantly above minimum', async () => {
      ...
      await bondToken.approve(bonding.address, minimumBondableValue.muln(2), { from: members[0] })
  ...
  describe('openKeep', async () => {
    ...
    it('reverts if no member candidates are registered', async () => {
      await expectRevert(
        keepFactory.openKeep(groupSize, threshold, keepOwner, bond, stakeLockDuration, {
          value: feeEstimate,
        }),
        'No pool found for the application'
      )
    ...
    it('opens keep with multiple members and emits event', async () => {
      ...
      const eventList = await keepFactory.getPastEvents('FullyBackedECDSAKeepCreated', {
      ...
      assert.sameMembers(ev.members, [members[0], members[1], members[2]], 'incorrect keep members')
    ...
    it('opens bonds for keep', async () => {
      ...
      expect(await bonding.bondAmount(members[0], keepAddress, keepAddress)).to.eq.BN(
        singleBond,
        'invalid bond value for members[0]'
      )

      expect(await bonding.bondAmount(members[1], keepAddress, keepAddress)).to.eq.BN(
        singleBond,
        'invalid bond value for members[1]'
      )

      expect(await bonding.bondAmount(members[2], keepAddress, keepAddress)).to.eq.BN(
        singleBond,
    ...
    it('reverts if not enough member candidates are registered', async () => {
      ...
      await expectRevert(
        keepFactory.openKeep(requestedGroupSize, threshold, keepOwner, bond, stakeLockDuration, {
          from: application,
          value: feeEstimate,
        }),
    ...
    it('requests new random group selection seed from random beacon', async () => {
      ...
      assert.equal(await randomBeacon.requestCount.call(), 1, 'incorrect number of beacon calls')
    ...
    it('reverts when honest threshold is greater than the group size', async () => {
      ...
      await expectRevert(
        keepFactory.openKeep(groupSize, honestThreshold, keepOwner, bond, stakeLockDuration, {
          from: application,
          value: feeEstimate,
        }),
    ...
    it('works when honest threshold is equal to the group size', async () => {
      ...
      await keepFactory.openKeep(groupSize, honestThreshold, keepOwner, bond, stakeLockDuration, {
        from: application,
        value: feeEstimate,
      })
    ...
    it('records the keep address and opening time', async () => {
      ...
      const factoryKeepOpenedTime = await keepFactory.getKeepOpenedTimestamp(keepAddress)
  ...
  describe('newGroupSelectionSeedFee', async () => {
    ...
    it('evaluates reseed fee for empty pool', async () => {
      ...
      expect(reseedFee).to.eq.BN(newEntryFee, 'reseed fee should equal new entry fee')
    })
  ...
  describe('getKeepAtIndex', async () => {
    ...
    it('returns keep at index', async () => {
      ...
      assert.equal(keep0.address, atIndex0, 'incorrect keep address returned for index 0')
      assert.equal(keep1.address, atIndex1, 'incorrect keep address returned for index 1')
    })
  })
  ...
  describe('banKeepMembers', async () => {
    ...
    it('bans keep members in a pool they are registered', async () => {
      ...
      for (let i = 0; i < operators.length; i++) {
        ...
        if (keepMembers.includes(operator)) {
          assert.isFalse(await keepFactory.isOperatorRegistered(operator, application))
    ...
    it('does not ban keep members in a pool they are not registered', async () => {
      ...
      const application2 = '0x0000000000000000000000000000000000000002'
      const pool2Address = await keepFactory.createSortitionPool.call(application2)
      await keepFactory.createSortitionPool(application2)
    ...
    it('emits events', async () => {
      ...
      assert.equal(eventList.length, keepMembers.length, 'incorrect number of emitted events')
  ...
  describe('getSortitionPoolWeight', async () => {
    ...
    it('returns pool weight if pool exists for application', async () => {
      ...
      expect(poolWeight, 'incorrect sortition pool weight').to.eq.BN(expectedPoolWeight)
    })
  ...
  async function initializeMemberCandidates(operators = members, unbondedValue) {
  ...
  async function delegate(operator, beneficiary, authorizer, unbondedValue) {
    ...
    await bondToken.approve(bonding.address, unbondedValue || minimumDelegationValue, {
      from: operator,
    })
  ...
  async function setUnbondedValue(operator, unbondedValue) {
    ...
    if (initialUnbondedValue.eq(unbondedValue)) {
      ...
      await bonding.withdraw(initialUnbondedValue.sub(unbondedValue), operator, { from: operator })
    } else {
  ...
  async function registerMemberCandidates(operators = members, app = application) {
    for (let i = 0; i < operators.length; i++) {
    ...
    const pool = await FullyBackedSortitionPool.at(await keepFactory.getSortitionPool.call(app))
