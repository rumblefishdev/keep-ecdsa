...
describe('BondedECDSAKeepFactory', function () {
  ...
  describe('registerMemberCandidate', async () => {
    ...
    it('inserts operators to different pools', async () => {
      ...
      await keepBonding.authorizeSortitionPoolContract(members[0], signerPool1Address, {
        from: authorizers[0],
      })
      await keepBonding.authorizeSortitionPoolContract(members[1], signerPool2Address, {
        from: authorizers[1],
      })
  ...
  describe('openKeep', async () => {
    ...
    it('opens bonds for keep', async () => {
      ...
      expect(await keepBonding.bondAmount(members[0], keepAddress, keepAddress)).to.eq.BN(
        singleBond,
        'invalid bond value for members[0]'
      )

      expect(await keepBonding.bondAmount(members[1], keepAddress, keepAddress)).to.eq.BN(
        singleBond,
        'invalid bond value for members[1]'
      )

      expect(await keepBonding.bondAmount(members[2], keepAddress, keepAddress)).to.eq.BN(
        singleBond,
    ...
    async function createDepositAndRegisterMembers(memberCount, unbondedAmount) {
      ...
      for (let i = 0; i < memberCount; i++) {
        ...
        await bondToken.approve(keepBonding.address, unbondedAmount, { from: operator })
  ...
  describe('getSortitionPoolWeight', async () => {
    ...
    it('returns pool weight if pool exists for application', async () => {
      ...
      expect(poolWeight).to.eq.BN(expectedPoolWeight, 'incorrect sortition pool weight')
    })
