...
describe('AbstractBonding', function () {
  ...
  before(async () => {
    ...
    await abstractBonding.authorizeSortitionPoolContract(operator, sortitionPool, {
      from: authorizer,
    })
  ...
  describe('deposit', async () => {
    const value = new BN(1e12)
    const expectedUnbonded = value
    it('registers unbonded value', async () => {
      await bondToken.mint(operator, value)
      await bondToken.approve(abstractBonding.address, value, { from: operator })

      await stakingInfoStub.setBeneficiary(operator, beneficiary)
      await abstractBonding.deposit(operator, value, { from: operator })
    ...
    it('sums deposits', async () => {
      ...
      await bondToken.mint(operator, value1.add(value2))
      await bondToken.approve(abstractBonding.address, value1.add(value2), { from: operator })

      await stakingInfoStub.setBeneficiary(operator, beneficiary)

      await abstractBonding.deposit(operator, value1, { from: operator })
      expect(
        await abstractBonding.availableUnbondedValue(operator, bondCreator, sortitionPool)
      ).to.eq.BN(value1, 'invalid unbonded value after first deposit')

      await abstractBonding.deposit(operator, value2, { from: operator })
      expect(
        await abstractBonding.availableUnbondedValue(operator, bondCreator, sortitionPool)
      ).to.eq.BN(value1.add(value2), 'invalid unbonded value after second deposit')
    })
    ...
    it('emits event', async () => {
      ...
      await bondToken.approve(abstractBonding.address, value, { from: operator })
      await stakingInfoStub.setBeneficiary(operator, beneficiary)
      const receipt = await abstractBonding.deposit(operator, value, { from: operator })
    ...
    it('reverts if beneficiary is not defined', async () => {
      ...
      await expectRevert(
        abstractBonding.deposit(operator, 0, { from: operator }),
  ...
  describe('createBond', async () => {
    ...
    it('creates bond', async () => {
      ...
      await abstractBonding.createBond(operator, holder, reference, value, sortitionPool, {
        from: bondCreator,
      })
      ...
      const lockedBonds = await abstractBonding.bondAmount(operator, holder, reference)
    ...
    it('creates two bonds with the same reference for different operators', async () => {
      ...
      await bondToken.mint(operator2, value)
      await bondToken.approve(abstractBonding.address, value, { from: operator2 })
      await abstractBonding.deposit(operator2, value, { from: operator2 })

      await stakingInfoStub.authorizeOperatorContract(operator2, bondCreator)
      await abstractBonding.authorizeSortitionPoolContract(operator2, sortitionPool, {
        from: authorizer2,
      })
      await abstractBonding.createBond(operator, holder, reference, bondValue, sortitionPool, {
        from: bondCreator,
      })
      await abstractBonding.createBond(operator2, holder, reference, bondValue, sortitionPool, {
        from: bondCreator,
      })
      ...
      const lockedBonds1 = await abstractBonding.bondAmount(operator, holder, reference)
      expect(lockedBonds1).to.eq.BN(bondValue, 'unexpected bond value 1')

      const lockedBonds2 = await abstractBonding.bondAmount(operator2, holder, reference)
    ...
    it('fails to create two bonds with the same reference for the same operator', async () => {
      ...
      await expectRevert(
        abstractBonding.createBond(operator, holder, reference, bondValue, sortitionPool, {
          from: bondCreator,
        }),
    ...
    it('fails if insufficient unbonded value', async () => {
      ...
      await expectRevert(
        abstractBonding.createBond(operator, holder, 0, bondValue, sortitionPool, {
          from: bondCreator,
        }),
  ...
  describe('reassignBond', async () => {
    ...
    beforeEach(async () => {
      await bondToken.mint(operator, bondValue)
      await bondToken.approve(abstractBonding.address, bondValue, { from: operator })
      await stakingInfoStub.setBeneficiary(operator, beneficiary)
      await abstractBonding.deposit(operator, bondValue, { from: operator })
      await abstractBonding.createBond(operator, holder, reference, bondValue, sortitionPool, {
        from: bondCreator,
      })
    })
    ...
    it('reassigns bond to a new holder and a new reference', async () => {
      ...
      let lockedBonds = await abstractBonding.bondAmount(operator, holder, reference)
      expect(lockedBonds).to.eq.BN(0, 'invalid locked bonds')

      lockedBonds = await abstractBonding.bondAmount(operator, holder, newReference)
      expect(lockedBonds).to.eq.BN(0, 'invalid locked bonds')

      lockedBonds = await abstractBonding.bondAmount(operator, newHolder, reference)
      expect(lockedBonds).to.eq.BN(0, 'invalid locked bonds')

      lockedBonds = await abstractBonding.bondAmount(operator, newHolder, newReference)
      expect(lockedBonds).to.eq.BN(bondValue, 'invalid locked bonds')
    })

    it('reassigns bond to the same holder and a new reference', async () => {
      await abstractBonding.reassignBond(operator, reference, holder, newReference, {
        from: holder,
      })
    ...
    it('reassigns bond to a new holder and the same reference', async () => {
      await abstractBonding.reassignBond(operator, reference, newHolder, reference, {
        from: holder,
      })
    ...
    it('fails if sender is not the holder', async () => {
      await expectRevert(
        abstractBonding.reassignBond(operator, reference, newHolder, newReference, {
          from: accounts[0],
        }),
        'Bond not found'
      )
    ...
    it('fails if reassigned to the same holder and the same reference', async () => {
      ...
      await bondToken.approve(abstractBonding.address, bondValue, { from: operator })
      await abstractBonding.deposit(operator, bondValue, { from: operator })
      await abstractBonding.createBond(operator, holder, newReference, bondValue, sortitionPool, {
        from: bondCreator,
      })

      await expectRevert(
        abstractBonding.reassignBond(operator, reference, holder, newReference, {
          from: holder,
        }),
        'Reference ID not unique for holder and operator'
      )
  ...
  describe('freeBond', async () => {
    ...
    beforeEach(async () => {
      await bondToken.mint(operator, initialUnboundedValue)
      await bondToken.approve(abstractBonding.address, initialUnboundedValue, { from: operator })
      await stakingInfoStub.setBeneficiary(operator, beneficiary)
      await abstractBonding.deposit(operator, initialUnboundedValue, { from: operator })
      await abstractBonding.createBond(operator, holder, reference, bondValue, sortitionPool, {
        from: bondCreator,
      })
    })
  ...
  describe('seizeBond', async () => {
    ...
    it('transfers whole bond amount to destination account', async () => {
      ...
      const expectedBalance = web3.utils.toBN(await bondToken.balanceOf(destination)).add(amount)

      await abstractBonding.seizeBond(operator, reference, amount, destination, {
        from: holder,
      })

      const actualBalance = await bondToken.balanceOf(destination)
    ...
    it('emits event', async () => {
      ...
      const receipt = await abstractBonding.seizeBond(operator, reference, amount, destination, {
        from: holder,
      })
  ...
  describe('authorizeSortitionPoolContract', async () => {
    it('reverts when operator is not an authorizer', async () => {
      ...
      await expectRevert(
        abstractBonding.authorizeSortitionPoolContract(operator, sortitionPool, {
          from: authorizer1,
        }),
        'Not authorized'
      )
    ...
    it('should authorize sortition pool for the provided operator', async () => {
      ...
      assert.isTrue(
        await abstractBonding.hasSecondaryAuthorization(operator, sortitionPool),
  ...
  describe('deauthorizeSortitionPoolContract', async () => {
    it('reverts when operator is not an authorizer', async () => {
      ...
      await expectRevert(
        abstractBonding.deauthorizeSortitionPoolContract(operator, sortitionPool, {
          from: authorizer1,
        }),
        'Not authorized'
      )
    ...
    it('should deauthorize sortition pool for the provided operator', async () => {
      ...
      await abstractBonding.deauthorizeSortitionPoolContract(operator, sortitionPool, {
        from: authorizer,
      })
      assert.isFalse(
        await abstractBonding.hasSecondaryAuthorization(operator, sortitionPool),
    ...
    describe('withdrawBond', async () => {
      ...
      it('emits event', async () => {
        ...
        const receipt = await abstractBonding.withdrawBondExposed(value, operator, {
          from: operator,
        })
